...
/// </summary>
public partial class PaymentProcessedEventMessageProcessor(
    IEventDispatcher dispatcher,
    ILogger<PaymentProcessedEventMessageProcessor> logger) : IMessageProcessor
{
    /// <summary>
    ...
    {
        if (!TryDeserializeEvent(body, out PaymentProcessedEvent? integrationEvent))
        {
    ...
    private bool TryDeserializeEvent(ReadOnlyMemory<byte> body, [NotNullWhen(true)] out PaymentProcessedEvent? integrationEvent)
    {
        try
        {
            integrationEvent = JsonSerializer.Deserialize<PaymentProcessedEvent>(body.Span);
        }
    ...
    private async Task<MessageProcessingResult> DispatchAsync(
        PaymentProcessedEvent integrationEvent,
        CancellationToken cancellationToken)
    {
        try
        {
    ...
    [LoggerMessage(
        ...
        Message = "Falha ao processar PaymentProcessedEvent para usuário {UserId}, mensagem será reenfileirada")]
